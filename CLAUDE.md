...
- `make` / `make dist` — generate `src/build-info.ts` (stamps commit + version) then build to `dist/`

## Architecture

Six core components:

1. **Controller** (`src/index.ts`) — Entry point. Orchestrates parsing and rendering, serves HTML via `Bun.serve()`, provides REST API to request slides by number and a `GET /health` JSON endpoint. Creates Shiki highlighter at startup. Pre-processes Excalidraw images before rendering. Supports interactive reload (`r`) and quit (`q`) via raw stdin when running in a TTY. Resolves `version` and `commit` at startup from `src/build-info.ts` (generated by Makefile) with a live git fallback for dev mode.
