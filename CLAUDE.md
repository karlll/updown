...
- `make` / `make dist` — build to `dist/` (stamps commit + version via `--define`)
- `make standalone` — build self-contained binary to `dist/updown`
- `make standalone-<platform>` — cross-compile for a specific target (e.g. `standalone-darwin-arm64`, `standalone-linux-x64`)

## Architecture

Six core components:

1. **Controller** (`src/index.ts`) — Entry point. Parses and validates CLI arguments synchronously before any async work (exits immediately with help text on bad/missing args). Orchestrates parsing and rendering, serves HTML via `Bun.serve()`, provides REST API to request slides by number and a `GET /health` JSON endpoint. Creates Shiki highlighter at startup. Pre-processes Excalidraw images before rendering. Supports interactive reload (`r`) and quit (`q`) via raw stdin when running in a TTY. Resolves `version` and `commit` from `BUILD_VERSION`/`BUILD_COMMIT` constants injected via `--define` at build time, with a live git + `package.json` fallback for dev mode.
