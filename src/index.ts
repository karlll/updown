...
// Resolve version and commit — prefer static build-info (generated by Makefile),
// fall back to live git + package.json when running in dev mode.
let appVersion = "unknown";
let appCommit = "unknown";
try {
  const info = await import("./build-info.ts");
  appVersion = info.VERSION;
  appCommit = info.COMMIT;
} catch {
  try {
