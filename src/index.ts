...
// Parse CLI arguments synchronously first — before any async work — so we can
// exit immediately on bad/missing args without hanging on git or dynamic imports.
// Bun.argv is always [runtime, script-or-binary, ...userArgs] in both script mode
// (bun src/index.ts) and compiled binary mode (binary path appears twice in argv).
...
const HELP_TEXT = `\
updown — render a Markdown file as a slideshow in the browser

Usage:
  updown [options] <path-to-markdown-file>

Options:
  --plantuml-jar <path>   Path to the PlantUML JAR file (enables PlantUML diagrams)
  --theme <dir>           Path to an external theme directory (repeatable)
  -v, --version           Print version and exit
  -h, --help              Show this help text

Examples:
  updown slides.md
  updown --plantuml-jar /opt/plantuml.jar slides.md
  updown --theme ./my-theme slides.md`;
...
// Build-time constants injected via --define when building with make.
// Falls back to live git + package.json in dev mode (bun src/index.ts).
declare const BUILD_VERSION: string;
declare const BUILD_COMMIT: string;
let appVersion = "unknown";
let appCommit = "unknown";
try { appVersion = BUILD_VERSION; } catch { /* not set: dev mode */ }
try { appCommit = BUILD_COMMIT; } catch { /* not set: dev mode */ }
if (appVersion === "unknown") {
  try {
    appVersion = (await Bun.file(new URL("../package.json", import.meta.url)).json()).version;
  } catch { /* leave as unknown */ }
}
if (appCommit === "unknown") {
  try {
    appCommit = (await Bun.$`git rev-parse --short HEAD`.quiet().text()).trim();
  } catch { /* not a git repo or git unavailable */ }
}

if (showVersion) {
  console.log(`${appVersion} (${appCommit})`);
  process.exit(0);
}

// Validate required file argument (skipped above for -v).
import { existsSync } from "node:fs";
if (!filePath) {
  console.error("Error: missing required argument <path-to-markdown-file>\n");
  console.error(HELP_TEXT);
  process.exit(1);
}
if (!filePath.endsWith(".md") && !filePath.endsWith(".markdown")) {
  console.error(`Error: "${filePath}" does not appear to be a Markdown file (.md or .markdown)\n`);
  console.error(HELP_TEXT);
  process.exit(1);
}
